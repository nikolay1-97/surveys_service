...
export class AnswersOptionsRepository {
  ...
  async getStatBySurveyId(survey_id: number) {
    try {
      ...
      for (let cnt = 0; cnt <= surveyStat.length - 1; cnt++) {
        const email = surveyStat[cnt]['email'];
        const question = surveyStat[cnt]['question'];
        const answer = surveyStat[cnt]['answer'];
        const option = surveyStat[cnt]['option'];
        const type = surveyStat[cnt]['type'];
  ...
  async getStat() {
    try {
      ...
      const res = {};
      for (let cnt = 0; cnt <= surveyStat.length - 1; cnt++) {
        const survey = surveyStat[cnt]['survey'];
        const email = surveyStat[cnt]['email'];
        const question = surveyStat[cnt]['question'];
        const answer = surveyStat[cnt]['answer'];
        const option = surveyStat[cnt]['option'];
        ...
        if (email in res) {
          if (survey in res[email]['surveys']) {
            if (question in res[email]['surveys'][survey]['questions'])
              res[email]['surveys'][survey]['questions'][question][
                'options'
              ].push(option);
            else {
              res[email]['surveys'][survey]['questions'][question] = {
          ...
          res[email]['surveys'][survey]['questions'][question]['options'].push(
            option,
          );
        }
      }
